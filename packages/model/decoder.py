...
class Decoder(nn.Module):
    def __init__(self, in_channels, latent_dim, adjacency_list, seq_len = 64):
        ...
        self.res_block1 = ResNet(in_channels * 4, in_channels * 4, adjacency_list, stride=2, is_transpose=True,)
    ...
    def forward(self, z):
        x = self.fc(z)
        x = x.reshape(x.size(0), self.seq_len // 8, len(self.adjacency_list), self.in_channels * 4)
        x = self.res_block1(x)
        x = self.res_block2(x)
        x = self.res_block3(x)
        return x