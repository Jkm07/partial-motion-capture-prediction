...
def get_rotation_metrics(actual, expected, disable_joints: list[int]):
    ...
    l2lq = get_l2lq(actual, expected, selected_nodes)
    l2q = get_l2q(actual, expected, selected_nodes)
    return l2lq, l2q
...
def get_l2lq(actual, expected, selected_nodes: slice):
...
def get_l2q(actual, expected, selected_nodes: slice):
...
def prepare_data_for_l2q(matrix):
    ...
    matrix = get_global_rotations(matrix)
    return math_utils.matrix9D_to_quat_torch(matrix)

def get_global_rotations(rotations):
    hierarchy = get_default_hierarchy()
    rotations = rotations.clone()
    rotate_node_with_childs(rotations, torch.eye(3).cuda(), 0, hierarchy[0])
    return rotations

def rotate_node_with_childs(rotations: torch.Tensor, parent_rotation: torch.Tensor, curr_rot: int, curr_node: Node) -> torch.Tensor:
    curr_rot_slice = (..., curr_rot, slice(None), slice(None))
    rotations[curr_rot_slice] = rotations[curr_rot_slice] @ parent_rotation
    for child in curr_node.children:
        if child.type == 'End':
            continue
        curr_rot = rotate_node_with_childs(rotations, rotations[curr_rot_slice], curr_rot + 1, child)
    return curr_rot
